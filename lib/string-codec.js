...
encoder = function(str, algo) {
  return bufferEncoder(new Buffer(str), algo).toString();
};
  ...
  switch (algo) {
    ...
    case 'base91':
      if (hex = hex_parse(buf.toString())) {
        return base91.encode(new Buffer(hex, 'hex'));
      }
      break;
    case 'ascii':
      if (hex = hex_parse(buf.toString())) {
    ...
    case 'unixtime':
      return new Buffer(Date.parse(buf.toString()).toString(10));
    case 'lower':
...
decoder = function(str, algo) {
  return bufferDecoder(new Buffer(str), algo).toString();
};
...
bufferDecoder = function(buf, algo) {
  ...
  switch (algo) {
    ...
    case 'base91':
      return new Buffer(base91.decode(buf).toString('hex'));
    case 'ascii':
    ...
    case 'unixtime':
      return new Buffer(new Date(parseInt(buf.toString())).toString());
    case 'md5':
